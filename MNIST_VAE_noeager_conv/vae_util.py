...
class VariantionalAutoencoder(object):
  ...
  def build(self):
    ...
    net = slim.fully_connected(self.x, 600, scope='enc_fc1', activation_fn=tf.nn.elu)
    net = slim.dropout(net, 0.75, scope='dropout1')
    net = slim.fully_connected(net, 400, scope='enc_fc2', activation_fn=tf.nn.elu)
    net = slim.dropout(net, 0.75, scope='dropout2')
    f3 = slim.fully_connected(net, 200, scope='enc_fc3', activation_fn=tf.nn.elu)
    net = slim.dropout(net, 0.75, scope='dropout6')
    ...
    # Decode
    # z -> x_hat
    net = slim.fully_connected(self.z, 200, scope='dec_fc1', activation_fn=tf.nn.elu)
