...
useConvDiscriminator = useConvEncoder
latent_dim = 32
...
y_discriminator = build_discriminator()
y_discriminator.compile(loss='binary_crossentropy', optimizer=optimizer)
y_discriminator.trainable = False
...
x_x_disc = x_discriminator(x_hat)
x_y_disc = y_discriminator(x_y_hat)
y_x_disc = x_discriminator(y_x_hat)
y_y_disc = y_discriminator(y_hat)
...
x_y_discriminator = tf.keras.Model(x, x_y_disc)
x_y_discriminator.compile(loss=loss_function, optimizer=optimizer)

y_x_discriminator = tf.keras.Model(y, y_x_disc)
y_x_discriminator.compile(loss=loss_function, optimizer=optimizer)

y_y_discriminator = tf.keras.Model(y, y_y_disc)
y_y_discriminator.compile(loss=loss_function, optimizer=optimizer)
...
for epoch in range(epochs):
    ...
    for i in range(numBatches):
        ...
        if epoch > 5:
            ...
            losses[5][-1] += c_loss
            y_y_discriminator.train_on_batch(epoch_y, np.ones((batch_size, 1)))
            x_y_discriminator.train_on_batch(epoch_x, np.ones((batch_size, 1)))
            y_x_discriminator.train_on_batch(epoch_y, np.ones((batch_size, 1)))

            if epoch%2 == 0: # train disc half the time
                # train discriminator
                reconstructed_x = x_x_reconstructor.predict(epoch_x)
                d_loss_real = x_discriminator.train_on_batch(epoch_x, np.ones((batch_size, 1)))
                d_loss_fake = x_discriminator.train_on_batch(reconstructed_x, np.zeros((batch_size, 1)))
                d_loss = 0.5 * np.add(d_loss_real, d_loss_fake)
                losses[4][-1] += d_loss

                reconstructed_y = y_y_reconstructor.predict(epoch_y)
                y_discriminator.train_on_batch(reconstructed_y, np.ones((batch_size, 1)))
                y_discriminator.train_on_batch(epoch_y, np.zeros((batch_size, 1)))

                reconstructed_x_y = x_y_reconstructor.predict(epoch_x)
                y_discriminator.train_on_batch(reconstructed_x_y, np.ones((batch_size, 1)))

                reconstructed_y_x = y_x_reconstructor.predict(epoch_y)
                x_discriminator.train_on_batch(reconstructed_y_x, np.ones((batch_size, 1)))
